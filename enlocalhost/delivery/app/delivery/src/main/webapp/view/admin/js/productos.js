$(function () {
    ...
    $("#ajax_wait").show();
    $.ajax({
        url: "http://localhost:8080/delivery/rest/categorias/cbo",
        dataType: "json",
        type: "GET",
        success: function (result) { //alert(JSON.stringify(result));
            ...
            if (result.ok) {
                ...
                // datos iniciales para diálogo dlg_search
                $.ajax({
                    url: "http://localhost:8080/delivery/rest/productos/search",
                    dataType: "json",
                    type: "GET",
...
function productosQry(carga) {
    ...
    $("#ajax_wait").show();
    $.ajax({
        url: "http://localhost:8080/delivery/rest/productos/qry/" + idcategoria,
        dataType: "json",
        type: "GET",
        data: {
            numPag: pagActual - 1,
            ...
            if (result.hasOwnProperty("msg")) {
                ...
                for (let i = 0; i < result.rows.length; ++i) {
                    ...
                    qry += "<tr id=\"" + result.rows[i][0] + "\">" +
                            ...
                            "</td>" +
                            "<td class=\"col-1\">" +
                            "<img src=\"http://localhost:8080/delivery/rest/productos/foto/" + result.rows[i][0] + "\" " + "alt=\"\" style=\"max-height: 50px\">" +
                            "</td>" +
...
function productosGet() {
    ...
    $.ajax({
        url: "http://localhost:8080/delivery/rest/productos/get/" + idproducto,
        dataType: "json",
        type: "GET",
        success: function (result) { //alert(JSON.stringify(result));
            if (result.ok) {
                ...
                $("#dlg_productos_imagen_view").html("<img src=\"http://localhost:8080/delivery/rest/productos/foto/" + result.productos.idproducto + "\" " + "alt=\"\" style=\"max-height: 50px\">");
...
function productosInsUpd() {
    ...
    $.ajax({
        url: "http://localhost:8080/delivery/rest/productos/insUpd",
        dataType: "json",
        type: "POST",
        contentType: false,
        processData: false,
...
function dlg_confirm_confirm() {
    ...
    if (accion === "DEL") {
        $.ajax({
            url: "http://localhost:8080/delivery/rest/productos/del/" + ids,
            type: "DELETE",
